...
#define minTempVal 0.0f
#define maxTempVal 120.0f

uint32_t Change_me(float *temperature);
uint8_t Scale_Hue(float *temperature);
uint32_t HSV(uint8_t h, uint8_t s, uint8_t v);  //OK
uint32_t convertRGB(const uint8_t src, const uint8_t dst, uint32_t color); // OK
